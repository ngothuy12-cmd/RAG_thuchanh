...
Không thay đổi logic ở bước này.
"""
"""
RAG logic module for Buổi 08.
Baseline sao chép từ Buổi 07 (Semantic Retrieval Baseline).
Sử dụng cấu hình và storage nội bộ của Buổi 08.
"""
import json
import argparse
import sys
        ...
        for i, chunk in enumerate(chunks):
            ...
            while True:
                try:
                    ...
                    if (i + 1) % 10 == 0:
                        print(f"Đã nhúng xong {i + 1}/{len(chunks)} chunks...")
                    time.sleep(3.5) # 15 RPM ~ 4 seconds per request to avoid rate limit
                    break
                except Exception as e:
                    if "429" in str(e):
                        print(f"Đang chờ API (Rate Limit) cho chunk {chunk['chunk_id']}... nghỉ 10 giây")
                        time.sleep(10)
                    else:
